    ...
    >
      ...
      >
        <view
          ...
          :key="index"
          :class="'type ' + (item.groupType === groupType && 'type-active')"
          :data-value="item"
          @tap="selectType"
        >
          <view
            class="group-type-list"
            :data-item="item"
          >
    ...
    bindConfirmCreate(){
      ...
      promise
        .then(imResponse => {
          ...
          uni.navigateTo({
            url: `/tim-pages/chat/chat?conversationID=GROUP${groupID}`
          });
        })
        .catch(() => {
          uni.showToast({
