...
export default {
  ...
  onLoad(options){
    ...
    uni.$TUIKit.getConversationProfile(conversationID).then(res => {
      ...
      this.setData({
        ...
        isShow: "GROUP" === conversation.type
      });
    });
  },
  mounted(){},
  ...
  methods: {
    ...
    goBack(){
      ...
      if(
        "tim-pages/create-conversation/create" === pages[pages.length - 2].route ||
				"tim-pages/create-group/create" === pages[pages.length - 2].route ||
				"tim-pages/join-group/join" === pages[pages.length - 2].route
      ){
        uni.navigateBack({
          delta: 2
        });
