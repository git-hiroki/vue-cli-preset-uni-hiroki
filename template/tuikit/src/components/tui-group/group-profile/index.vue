<template>
  <view>
    <view
      class="group-information-box"
      @show="getgroupProfile"
    >
      <view class="group-box">
        ...
        <view @tap="showMoreHandler">
          <text class="group-member">
            聊天成员:{{conversation.groupProfile.memberCount}}人
          </text>
          <image
            v-if="notShow"
            class=" icon-right"
            src="~@/static/assets/tim//down.svg"
          ></image>
          <image
            v-if="isShow"
            class=" icon-right"
            src="~@/static/assets/tim//up.svg"
          ></image>
        </view>
      </view>
      <view
      ...
      >
        <view
          v-for="(item, index) in groupmemberprofile"
          :key="index"
          class="box"
          v-if="index < 3"
          :data-value="item"
        ...
        >
          <image
            class="profile-box"
            src="~@/static/assets/tim//show.svg"
            @tap="showMoreMember"
          ></image>
          <text class="nick-box">
            更多
        ...
        <view class="left-box">
          ...
          >
            <image
              class="addmember"
              src="~@/static/assets/tim//addgroup.svg"
              @tap="addMember"
            ></image>
            <text class="addmember-text">
              添加成员
            </text>
          </view>
          <view class="box-group-quit">
            <image
              class="quitgroup"
              src="~@/static/assets/tim//quitgroup.svg"
              @tap.stop="quitGroup"
            ></image>
            <text class="quitgroup-text">
              退出群聊
            </text>
          </view>
        </view>
    ...
    <view class="pop-container">
      <view
        class="quitpop-mask"
        v-if="quitpopupToggle"
      >
        <view class="quitpop">
          <view class=" quit-box">
            <view class="text-box">
              <text class="confirmQuitgroup-text">
                退出群聊后会同步删除历史聊天记录，是否要退出群聊？
              </text>
            </view>
            <view
              class="text-box-qiut"
              @tap.stop="quitgroupConfirm"
            >
              <text class="quitgroup-confirm">
                退出
              </text>
            </view>
            <view
              class="text-box-cancle"
              @tap="quitgroupAbandon"
            >
              <text class="quitgroup-abandon">
                取消
              </text>
            </view>
          </view>
    ...
    >
      <view class="popup">
        ...
        <view class="popup-footer">
          <button
            class=" popup-footer-button submit"
            @tap.stop="submit"
          >
            确认
          </button>
          <button
            class="popup-footer-button cancel"
            @tap.stop="close"
          >
            取消
          </button>
    ...
    showMoreHandler(){
      uni.$TUIKit
        ...
        .then(imResponse => {
          ...
          if("Private" === this.conversation.groupProfile.type){
            this.setData({
              addShow: true
            });
          }
