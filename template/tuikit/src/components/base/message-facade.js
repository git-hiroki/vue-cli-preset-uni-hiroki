...
export function parseGroupSystemNotice(message){
  const {
    ...
    groupName = payload.groupProfile.name || payload.groupProfile.groupID,
    {
      groupID
...
export function parseGroupTip(message){
  const {
    ...
    userName = message.nick || payload.userIDList.join(",");
  let tip,
    user;
  ...
  switch(payload.operationType){
    ...
    case 256:
      ...
      tip = "结束群聊" === payload.text ? "结束群聊" : `"${user}" ${payload.text}`;
      break;
  }
...
export function parseAudio(message){
  const renderDom = {
    ...
    second: 0 === message.payload.second ? 1 : message.payload.second
  };
  return renderDom;
}