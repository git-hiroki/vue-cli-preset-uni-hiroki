import logger from "@/utils/logger"; // -----------------检测类型工具函数-----------------
...
export const isNumber = function(input){
  return input !== null && ("number" === typeof input && !isNaN(input - 0) || "object" === typeof input && input.constructor === Number);
};
/**
 * 检测input类型是否为string
 * @param {*} input 任意类型的输入
...
export const isObject = function(input){
  ...
  return input !== null && "object" === typeof input;
};
/**
...
export const isNativeClass = function(input){
  return "function" === typeof input && isNativeClassRegex.test(input.toString());
};
/**
...
export const date2ISOString = function(){
  const date = new Date(), // YYYY-MM-DDTHH:mm:ss.sssZ。时区总是UTC（协调世界时），加一个后缀“Z”标识。
    ...
    tempISOString = date.toISOString(), // 返回协调世界时（UTC）相对于当前时区的时间差值，单位为分钟。
    // 如果本地时区晚于协调世界时，则该差值为正值，如果早于协调世界时则为负值

    timezoneOffset = date.getTimezoneOffset(),
    currentTimeZoneInHour = timezoneOffset / 60;
  let replaceString = "";
...
export const autoincrementIndex = function(key){
  ...
  if("undefined" === typeof AIIList[key]){
    const dateInstance = new Date();
    let hoursString = `3${dateInstance.getHours()}`.slice(-2), // 小时数，用3占位，因为一天中不会出现30小时

      minuteString = `0${dateInstance.getMinutes()}`.slice(-2),
      secondString = `0${dateInstance.getSeconds()}`.slice(-2);
