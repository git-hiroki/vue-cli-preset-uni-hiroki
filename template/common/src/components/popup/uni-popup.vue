<template>
  <view v-if="showPopup" :class="classList" @touchmove.stop.prevent="clear">
...
<script>
import uniTransition from "@/components/popup/uni-transition";
/**
   * PopUp 弹出层
...
export default {
  ...
  computed: {
    classList(){
      return ["uni-popup"].concat(this.popupStore.state.class?.map(
        item => `uni-popup-${item}`
      ));
    }
  },
